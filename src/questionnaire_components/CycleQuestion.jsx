  ...
  return (
    <div className="px-4 md:px-8 py-6 space-y-6">
      {question.options.map((subQ, index) => (
        <div key={index}>
          <h3 className={`font-bold mb-3 ${isLight ? "text-[rgba(153,74,151,1)]" : "text-slate-50" } text-lg md:text-xl`}>
            {subQ.subq}
          </h3>
                ...
                <label
                  ...
                  onClick={() => {
                    if (isMulti) {
                      ...
                      // for single-select, set directly
                      setSelected((prev) => ({
                        ...prev,
                        [`cycle-${index}`]: o,
                      }));
                  ...
                  <div className="flex items-center gap-3 p-0.5">
                    {!isMulti && (
                      <input
                        ...
                        checked={isSelected}
                        onChange={() =>
                          setSelected((prev) => ({ ...prev, [`cycle-${index}`]: o }))
                        }
                        className="sr-only"
                      />
                    )}
                    ...
                    <span
                      className={`w-4 h-4 rounded-full flex items-center justify-center border-2 transition-all ${
                        isSelected
                          ? `border-[rgba(233,217,227,1)] ${isLight ? "bg-[rgba(233,217,227,1)]" : "bg-white/60"} shadow-md`
                          : `border-[rgba(233,217,227,0.3)] ${isLight ? "bg-transparent" : "bg-white/10"}`
                      }`}
                    >
                      {isSelected && <span className="w-2 h-2 rounded-full bg-white" />}
                    </span>

                    <h2 className={`font-semibold text-sm md:text-base ${isLight ? "text-[rgba(153,74,151,1)]" : "text-slate-50/90" }`}>{o}</h2>
                  </div>
                </label>
              );
            })}
