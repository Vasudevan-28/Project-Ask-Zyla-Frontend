...
export default function TrialChatMessage({ role, text, onSpeak, canSpeak, isSpeaking, showImage, bgTheme }) {
    ...
    />
        ...
        <div
          className={`rounded-2xl px-4 py-2 text-sm ${
            isUser
              ? ` ${bgTheme === "light" ? "text-black bg-[#E9D9E3]" : "bg-white/10 text-white/90"}  shadow-lg rounded-br-sm`
              : `${bgTheme === "light"  ? "text-gray-700 bg-[#E9D9E3]" : "bg-white/10 text-gray-200"} shadow-lg rounded-bl-sm`
          }`}
        >
          {text}
        </div>
