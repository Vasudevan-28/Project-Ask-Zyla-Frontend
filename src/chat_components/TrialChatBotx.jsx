...
export default function TrialChatbot({
  scrollerRef,
  messages,
  loading,
  ttsSupported,
  ...
  return (
    <div className="flex-1 flex flex-col p-1 mt-2 custom-scrollbar min-h-[600px] ">
      <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full min-h-0   ">
        ...
        >
          <div
            className={`w-full flex items-center h-16 gap-2 rounded-b-lg z-10 justify-between
                  ${bgTheme == "light" ? " bg-white" : "bg-white/10"}`}
          >
            <div className="flex items-center justify-center gap-1.5">
              <img
                src={ZylaChatPic}
                className="ml-2 md:ml-4 mr-1 w-12 h-12 rounded-md  border-white/30 object-cover"
                alt="Zyla"
              />
              <h1 className={`text-2xl font-bold ${bgTheme == "light" ? " text-[#1d0e2d]" : "text-white/90"}`}>
                Zyla
              </h1>
            </div>
            <div className="mr-6">
                {trialRemaining !== null && !trialOver && (
              <span className={`text-sm font-medium   ${bgTheme == "light" ? " text-[#1d0e2d]" : "text-white/90"}`}>
                Free chats left:{" "}
            ...
            {trialOver && (
              <span className={`text-sm font-medium text-red-500  ${bgTheme == "light" ? " text-[#1d0e2d]" : "text-white/90"}`}>
              ...
              <div className="flex-1 flex justify-end relative">
                ...
                <input
                  className={`w-full border text-sm focus:outline-none focus:ring-0 border-white/25 rounded-xl px-4 py-3 pr-14 relative z-10
                        ${bgTheme == "light" ? "bg-[#E9D9E3] placeholder-gray-600 text-slate-900 " : "bg-white/10 placeholder-gray-400 text-slate-50"}`}
                  ...
                  <button
                    ...
                    className={`absolute cursor-pointer right-3 top-1/2 transform -translate-y-1/2 p-2 rounded-lg transition-all duration-300 z-20 ${
                      isListening
                        ...
                        : `${bgTheme == 'light' ? "bg-[#B9A3C7] text-white hover:bg-white/30 hover:text-[#B9A3C7]" : "bg-white/10 text-slate-100 hover:bg-slate-100 hover:text-slate-700"} border-white/30 shadow-lg hover:shadow-xl`
                    } hover:scale-110`}
